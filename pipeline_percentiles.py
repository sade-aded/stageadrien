...
def fuzzy_match_team(target_team, available_teams, cutoff=0.5):
    ...
    Strategy:
    1. Exact case-insensitive substring match (like before)
    2. Normalized match (St John's -> st johns -> matches St. John's)
    ...
    for t in available_teams:
        if target_lower in t.lower() or t.lower() in target_lower:
            return t, None
    ...
    for n, orig in norm_map.items():
        if target_norm in n or n in target_norm:
            return orig, f"Matched '{target_team}' to '{orig}' (normalized match)."
